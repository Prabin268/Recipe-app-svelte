<script lang="ts">
	...
	type Route = '/homepage' | '/savedpage' | '/notificationpage' | '/profilepage' | '/addeditpage';
...
<aside
	class="sticky top-0 hidden max-h-screen flex-col border-r
  bg-white transition-all duration-300 md:flex
  {collapsed ? 'w-16' : 'w-56'}"
>
	<div class="flex cursor-pointer justify-end p-3">
		...
		<button
			...
			class="z-100 cursor-pointer rounded-md p-2 hover:bg-gray-100"
		>
			{#if collapsed}
	...
	<nav class="flex flex-col gap-3 px-2">
		{#each items as item}
			...
			>
				...
				<span
					class="overflow-hidden whitespace-nowrap transition-all duration-300 {collapsed
						? 'w-0 opacity-0'
						: 'w-auto opacity-100'}"
				>
					{item.label}
	...
	<div class="mt-auto mb-3 flex flex-col gap-2 px-2">
		<div
			class="flex cursor-pointer items-center gap-3 rounded-lg px-3 py-2 text-sm text-gray-600 hover:bg-green-100 hover:text-green-600"
		>
			<img src={$userStore?.photo_url} alt="pic" class="h-6 w-6 rounded-full" />

				<div class="flex flex-col text-left overflow-hidden whitespace-nowrap transition-all duration-300 {collapsed
						? 'w-0 opacity-0'
						: 'w-full opacity-100'}">
					<p class="text-xs font-medium">{$userStore?.display_name}</p>
